...
export async function POST(req: NextRequest) {
  try {
    ...
    // Resolve active branch with smart fallback to prevent false-positive inactive branch block
    let targetBranchId = branchId;
    let branch: any = targetBranchId
      ? await prisma.branch.findUnique({ where: { id: targetBranchId } })
      : null;

    if (!branch || branch.isActive === false) {
      // Find first active retail branch
      ...
      if (!activeBranch) {
        return NextResponse.json(
          { error: 'Cabang ini sedang nonaktif dan tidak ada cabang aktif yang tersedia.' },
          { status: 400 }
        );
      }
    ...
    const result = await prisma.$transaction(
      async (tx: any) => {
        ...
        const stockMap = new Map<string, any>(
          branchStocks.map((s: any) => [s.productId, s])
        );
        ...
        const pointRedeemValue = storeSetting
          ? Number(storeSetting.pointRedeemValue)
          : 100;
        const pointsEarnThreshold = storeSetting
          ? Number(storeSetting.pointsEarnThreshold)
          : 10000;
        const isLoyaltyActive = storeSetting
          ? storeSetting.isLoyaltyActive
          : true;
        ...
        if (memberId && isLoyaltyActive) {
          ...
          if (pointsUsed > 0) {
            if (member.points < pointsUsed) {
              throw new Error(
                `Member only has ${member.points} points available.`
              );
            }
        ...
        const pointsEarned =
          isLoyaltyActive && memberId && pointsEarnThreshold > 0
        ...
        // 3. Create Transaction record with verified cashierId and targetBranchId
        ...
        for (const vi of verifiedItems) {
          ...
          await tx.branchStock.upsert({
            ...
            create: {
              branchId: targetBranchId,
              productId: vi.productId,
              quantity: newBalance > 0 ? newBalance : 0,
            },
          });

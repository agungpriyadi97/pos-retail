...
export default function PosPage() {
  ...
  const handleCheckout = async () => {
    ...
    if (paymentMethod === 'CASH' && paidAmount < finalAmount) {
      setCheckoutError(
        `Jumlah bayar tunai (Rp ${paidAmount.toLocaleString()}) kurang dari total tagihan (Rp ${finalAmount.toLocaleString()}).`
      );
      return;
    }
  ...
  return (
    <DashboardLayout>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-6rem)]">
        ...
        <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-4 overflow-hidden">
          ...
          <div className="flex items-center gap-3 bg-slate-900 border border-slate-800 p-3 rounded-2xl shadow-sm">
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm shadow-md shadow-emerald-500/20 transition shrink-0"
            >
                ...
                {products.map((product) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-slate-900 border rounded-2xl p-3 flex flex-col justify-between transition cursor-pointer group ${
                        isOutOfStock
                    ...
                    >
                      <div>
                        ...
                        <h3 className="font-semibold text-sm text-white line-clamp-2 mb-1 group-hover:text-emerald-400 transition">
                          {product.name}
                        </h3>
        ...
        <div className="lg:col-span-5 xl:col-span-4 bg-slate-900 border border-slate-800 rounded-2xl flex flex-col h-full overflow-hidden shadow-xl">
          {/* Header */}
          <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/90">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-emerald-400" />
              <h2 className="font-bold text-white">Keranjang Kasir</h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-slate-500 text-sm space-y-2">
                ...
                <p>Klik produk di sebelah kiri untuk menambahkan ke keranjang.</p>
              </div>
            ) : (
              cart.map((item) => (
                <div
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm w-6 text-center text-white">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQuantity(item.product.id, 1)}
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-950/60 space-y-3">
            ...
            {selectedMember ? (
              <div className="bg-slate-900 border border-emerald-500/40 rounded-xl p-2.5 flex items-center justify-between">
                ...
                <div className="text-right">
                  <label className="flex items-center gap-1.5 text-xs text-slate-300 cursor-pointer">
                    ...
                    />
                    <span>Gunakan Poin Belanja</span>
                  </label>
                </div>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                {memberNotFound && (
                  <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs flex items-center justify-between">
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {(['CASH', 'QRIS', 'TRANSFER', 'DEBIT'] as const).map((method) => (
                <button
                  ...
                  className={`py-2 rounded-xl text-xs font-semibold border transition flex flex-col items-center justify-center gap-1 ${
                ...
                >
                  ...
                  {method === 'TRANSFER' && <ArrowRight className="w-4 h-4" />}
            ...
            {paymentMethod === 'CASH' && (
              <div className="space-y-2 pt-1">
                <div className="flex gap-1.5">
                  {[finalAmount, 10000, 20000, 50000, 100000].map((amt, idx) => (
                    <button
                      key={idx}
                      onClick={() => setPaidAmountInput(amt.toString())}
                      className="flex-1 bg-slate-950 border border-slate-800 hover:border-slate-700 py-1 rounded-lg text-[10px] font-mono text-slate-300"
                    >
                      {amt === finalAmount ? 'Uang Pas' : `Rp ${amt / 1000}k`}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-sm text-white font-mono placeholder-slate-600 focus:outline-none focus:border-emerald-500"
                    />
                  </div>

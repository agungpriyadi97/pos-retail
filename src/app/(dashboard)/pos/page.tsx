...
export default function PosPage() {
  ...
  const renderCartPanel = (isMobile: boolean = false) => (
    <div
      className={`bg-slate-900 border border-slate-800 rounded-2xl flex flex-col h-full overflow-hidden shadow-xl ${
        isMobile ? 'w-full max-h-[90vh]' : 'hidden md:flex lg:col-span-5 xl:col-span-4'
      }`}
    >
      {/* Header */}
      ...
      {/* Cart Item List */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5 sm:space-y-3 min-h-[160px]">
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-slate-500 text-xs sm:text-sm space-y-2">
      ...
      <div className="p-3 sm:p-4 border-t border-slate-800 bg-slate-950/60 space-y-2.5 sm:space-y-3 shrink-0">
      ...
      <div className="p-3 sm:p-4 border-t border-slate-800 bg-slate-900 space-y-2.5 sm:space-y-3 shrink-0">
        {checkoutError && (
            ...
            <div className="flex gap-1 overflow-x-auto pb-0.5">
              {[finalAmount, 10000, 20000, 50000, 100000].map((amt, idx) => (
                <button
                  ...
                  className="flex-1 min-w-[54px] bg-slate-950 border border-slate-800 hover:border-slate-700 py-1 rounded-lg text-[10px] font-mono text-slate-300"
                >
                  {amt === finalAmount ? 'Pas' : `Rp ${amt / 1000}k`}
                </button>
              ))}
            </div>
    ...
    <DashboardLayout>
      ...
      {isMobileCartOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex flex-col justify-end">
          ...
          <div className="relative z-10 w-full max-h-[90vh] overflow-y-auto rounded-t-3xl shadow-2xl">
